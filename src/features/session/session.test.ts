...
import { speedSchema, stepSchema, timeframeSchema } from "@/features/session/schema"
...
describe("session command schemas", () => {
  ...
  // A negative count is deliberately allowed through. The cursor is forward-only and the server
  // refuses it with CURSOR_IS_FORWARD_ONLY, which says what the rule is; failing it here would
  // turn that answer into a generic validation error.
  it("passes a negative step through to the server rather than answering for it", () => {
